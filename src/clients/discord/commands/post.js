...
const command = {
    ...
    execute: async ({ message, args }) => {
        ...
        try {
            ...
            let runStatus;
            let attempts = 0;
            const maxAttempts = 60; // 10 minutes (polling every 10 seconds)
                    ...
                    if (status === "completed" || status === "success") {
                        ...
                        if (!humanNodeDetected) {
                            ...
                            if (process.env.LANGGRAPH_STUDIO_URL) {
                                ...
                                await processingMessage.edit(`Your post is being processed in the human review step. ` +
            ...
            if (attempts >= maxAttempts) {
                const messageContent = "Timed out while waiting for post completion. The process may still be running. " +
                ...
                message.content.forEach((part) => {
                    if (part.type === "text" && part.text) {
                        ...
                        if (part.text.value.includes("# Report") || part.text.value.includes("## Report")) {
                            ...
                            const reportSection = sections.find((s) => s.startsWith("# Report") || s.startsWith("## Report"));
                            if (reportSection) {
                                report = reportSection.replace(/^# Report|^## Report/, "").trim();
                            }
                            // Look for relevant links
                            const linksSection = sections.find((s) => s.includes("Relevant URLs") || s.includes("Relevant Links"));
                            if (linksSection) {
                            ...
                            const imageSection = sections.find((s) => s.includes("Image Options"));
                            if (imageSection) {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Post ID', value: postId, inline: true }, { name: 'Platform', value: 'Twitter/LinkedIn', inline: true }, { name: 'Scheduled For', value: scheduleDisplay, inline: true }, { name: 'Original URL', value: url })
